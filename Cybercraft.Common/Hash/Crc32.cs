...
namespace Cybercraft.Common.Hash
{
	...
	public static class Crc32
	{
		...
		public static unsafe uint GetCrc(uint startCrc, byte[] data, int offset, int length)
		{
			if (length > 0)
			{
	    ...
	    public static uint Calculate(string fileName)
	    {
	        ...
	        unchecked
	        {
	            using (var fs = new FileStream(fileName, FileMode.Open, FileAccess.Read, FileShare.Read))
	            {
	                ...
	                while (bytesLeft > 0)
	                {
	                    int inputLength = (int)Math.Min(bytesLeft, bufSize);
	                    fs.Read(input, 0, inputLength);
        ...
        public static uint Calculate(Stream stream, long length = 0)
        {
            if (length == 0)
                length = stream.Length;

            const int bufSize = 1024 * 1024;
            const int maxQueueLength = 32*32;
            long bytesLeft = length;
            int blocks = (int)(length % bufSize == 0 ? length / bufSize : (length / bufSize) + 1);

            var queue = new BlockingCollection<byte[]>(maxQueueLength);
            ...
            var reader = Task.Run(() =>
            {
                while (bytesLeft > 0)
                {
                    ...
                    stream.Read(input, 0, inputLength);
                    queue.Add(input);
            ...
            Task.WaitAll(reader, hasher);

            /*var reader = new Thread(o => {
                while (bytesLeft > 0)
                {
                    int inputLength = (int)Math.Min(bytesLeft, bufSize);
