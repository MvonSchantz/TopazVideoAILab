...
{
    public class IrisLqModel : ModelBase
    {
        public override string Name => "Iris LQ-1";

        public override string TopazName => "iris-1";

        public override UpscaleAlgorithm UpscaleAlgorithm => UpscaleAlgorithm.Iris;
            ...
            bool auto = false, int revertCompression = -1, int recoverDetails = -1, int sharpen = -1, int reduceNoise = -1, int dehalo = -1, int antiAliasDeblur = -1, int recoverOriginalDetails = -1)
        {
            if (auto)
            {
                return $"irisLq1Auto{upscaleFactor}x";
            }
            else
            {
                return $"irisLq1-{revertCompression}-{recoverDetails}-{sharpen}-{reduceNoise}-{dehalo}-{antiAliasDeblur}-{recoverOriginalDetails}-{upscaleFactor}x";
            }
        }
